...
#if false && NETSTANDARD1_1_OR_GREATER
...
internal readonly struct ProbabilisticMap
{
    ...
    private static bool Contains(ReadOnlySpan<char> values, char ch) =>
        SpanHelpersHidden.NonPackedContainsValueType(
    ...
    {
        var valuesSpan = MemoryMarshalPolyfills.CreateReadOnlySpan(ref values, valuesLength);
        ...
        {
            return IndexOfAnySimpleLoop<IndexOfAnyAsciiSearcherHidden.DontNegate>(ref searchSpace, searchSpaceLength, valuesSpan);
        }
    ...
    {
        var valuesSpan = MemoryMarshal.CreateReadOnlySpan(ref values, valuesLength);

        if (IndexOfAnyAsciiSearcherHidden.IsVectorizationSupported &&
            !ShouldUseSimpleLoop(searchSpaceLength, valuesLength) &&
            IndexOfAnyAsciiSearcherHidden.TryIndexOfAnyExcept(ref searchSpace, searchSpaceLength, valuesSpan, out int index))
        {
            return index;
        }

        return IndexOfAnySimpleLoop<IndexOfAnyAsciiSearcherHidden.Negate>(ref searchSpace, searchSpaceLength, valuesSpan);
    }
...
#pragma warning restore IntrinsicsInSystemPrivateCoreLibAttributeNotSpecificEnough
    ...
    internal static int IndexOfAnySimpleLoop<TNegator>(ref char searchSpace, int searchSpaceLength, ReadOnlySpan<char> values)
        where TNegator : struct, IndexOfAnyAsciiSearcherHidden.INegator
    {
        ref char searchSpaceEnd = ref Unsafe.Add(ref searchSpace, searchSpaceLength);
        ref char cur = ref searchSpace;
