...
#if false && NETCOREAPP3_0_OR_GREATER
...
public static class IndexOfAnyAsciiSearcherHidden
{
    public struct AsciiState(Vector128<byte> bitmap, BitVector256Hidden lookup)
    {
        public Vector256<byte> Bitmap = Vector256.Create(bitmap, bitmap);
        public BitVector256Hidden Lookup = lookup;

        public readonly AsciiState CreateInverse() =>
            new(Bitmap.GetLower().OnesComplement(), Lookup.CreateInverse());
    }

    public struct AnyByteState(Vector128<byte> bitmap0, Vector128<byte> bitmap1, BitVector256Hidden lookup)
    {
    ...
    internal static unsafe void ComputeAnyByteState(ReadOnlySpan<byte> values, out AnyByteState state)
    {
        ...
        BitVector256Hidden lookupLocal = default;
        ...
        where TNegator : struct, INegator
    {
        ...
        if (IsVectorizationSupported)
        {
            ...
            if (TryComputeBitmap(asciiValues, (byte*)&state.Bitmap, out bool needleContainsZero))
            {
                ...
                state.Bitmap = Vector256.Create(state.Bitmap.GetLower(), state.Bitmap.GetLower());
        ...
        where TResultMapper : struct, IResultMapper<short, TResult>
    {
        ...
        Vector128<byte> bitmap = state.Bitmap.GetLower();
        ...
        where TResultMapper : struct, IResultMapper<byte, TResult>
    {
        ...
        Vector128<byte> bitmap0 = state.Bitmap0.GetLower();
        Vector128<byte> bitmap1 = state.Bitmap1.GetLower();
    ...
    private static Vector128<byte> IndexOfAnyLookupCore(Vector128<byte> source, Vector128<byte> bitmapLookup)
    {
        ...
        Vector128<byte> bitMask = Vector128.Shuffle(bitmapLookup, lowNibbles);
        ...
        Vector128<byte> bitPositions = Vector128.Shuffle(Vector128.Create(0x8040201008040201, 0).AsByte(), highNibbles);
        ...
        where TNegator : struct, INegator
    {
        ...
        Vector128<byte> row0 = Vector128.Shuffle(bitmapLookup0, lowNibbles);
        Vector128<byte> row1 = Vector128.Shuffle(bitmapLookup1, lowNibbles);

        Vector128<byte> bitmask = Vector128.Shuffle(Vector128.Create(0x8040201008040201).AsByte(), highNibbles);
        ...
        where TNegator : struct, INegator
    {
        ...
        return offsetInVector + (int)((uint)Unsafe.ByteOffset(ref searchSpace, ref current) / (uint)sizeof(T));
    }
        ...
        where TNegator : struct, INegator
    {
        ...
        return offsetInVector - Vector128<short>.Count + (int)((uint)Unsafe.ByteOffset(ref searchSpace, ref secondVector) / (uint)sizeof(T));
    }
        ...
        where TNegator : struct, INegator
    {
        ...
        {
            result = PackedSpanHelpersHidden.FixUpPackedVector256Result(result);
        }
        ...
        where TNegator : struct, INegator
    {
        ...
        // We matched within the second vector
        return offsetInVector - Vector256<short>.Count + (int)((uint)Unsafe.ByteOffset(ref searchSpace, ref secondVector) / (uint)sizeof(T));
    }
    ...
    private readonly unsafe struct IndexOfAnyResultMapper<T> : IResultMapper<T, int>
    {
        ...
        {
            return (int)((uint)Unsafe.ByteOffset(ref searchSpace, ref current) / (uint)sizeof(T));
        }
        ...
        public static int FirstIndexOverlapped<TNegator>(ref T searchSpace, ref T current0, ref T current1, Vector128<byte> result) where TNegator : struct, INegator
        {
            ...
            return offsetInVector + (int)((uint)Unsafe.ByteOffset(ref searchSpace, ref current0) / (uint)sizeof(T));
        }
